...
import 'package:video_page/video_cache/cache.dart';
...
class CachePreview extends StatefulWidget {
  ...
  CachePreview(String? indexUrl,String? bgIconUrl,String? titleName, VideoCacheServer cacheServer, this.height) : _store = _Store(indexUrl,bgIconUrl,titleName, cacheServer);
...
class _Store {
  ...
  _Store(this.indexUrl,this.bgIconUrl,this.titleName, this.cacheServer);

  Future<void> prepareCacheData() async {
    CacheInfo? cacheInfo = cacheServer.caches[indexUrl];
    Map<String,dynamic> map = Map();
    map["bgIconUrl"] = bgIconUrl;
    map["videoUrl"] = indexUrl;
    map["titleName"] =  titleName;
    var bodySave = json.encode(map);
    // UserSharedPreferences.setPrivateCache(bodySave);
    if (cacheInfo != null) {
      // a single file cache
